...
G29_TYPE GcodeSuite::G29() {
  ...
  // P = populate grid with specified Z value
    ...
    float init_val = 0.0f;
    if (do_init) {
      init_val = motion.raw_z(parser.value_linear_units());
  ...
  if (!seen_w && !faux && motion.homing_needed_error()) G29_RETURN(false, false);
  ...
  if (!g29_in_progress) {
    ...
    if (!faux) {
      motion.remember_feedrate_scaling_off();
        ...
        if (!abl.dryrun) {
          #if ENABLED(RTS_AVAILABLE)
            probe.preheat_for_probing(probe_extrusion_temp,
              TERN(EXTENSIBLE_UI, ExtUI::getLevelingBedTemp(), probe_bed_temp)
            );
